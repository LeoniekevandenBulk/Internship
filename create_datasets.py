...
for series in trainseries:
    ...
    if(dataset == 'Simple'):
        ...
        generate_dataset_medium(
            realisation_path, connections_path, locations_path, composition_change_path, driver_change_path, series,
            category, validation=False, normalization=normalization, one_hot_encoding=one_hot_encoding)

        print("Creating validationset for trainseries " + str(series))
        generate_dataset_medium(
            validation_path, connections_path, locations_path, composition_change_path, driver_change_path, series,
            category, validation=True, normalization=normalization, one_hot_encoding=one_hot_encoding)
